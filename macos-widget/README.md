...
changes, so **keep the Success Portal app open** (or open it periodically) to
...
3. Product Name: `SuccessPortalWidgetsHost`. Interface: **SwiftUI**. Language: **Swift**.
...
5. In the generated project, delete the default `ContentView.swift` and the
   `SuccessPortalWidgetsHostApp.swift` Xcode created, and drag in the file from
   this folder: `SuccessPortalWidgetsHost/SuccessPortalWidgetsHostApp.swift`.

## 2. Add the widget extension

1. **File → New → Target…**
2. Choose **Widget Extension**, click Next.
3. Product Name: `SuccessPortalWidgets`. Uncheck **"Include Live Activity"**.
...
5. Drag in both files from this folder's `SuccessPortalWidgets/` directory:
   - `WidgetData.swift`
   - `SuccessPortalWidgets.swift`
   Make sure both are added to the **SuccessPortalWidgets** target (checkbox in
...
1. Select the project in the navigator → select the **SuccessPortalWidgetsHost**
...
3. Repeat for the **SuccessPortalWidgets** (extension) target.
...
1. Select the **SuccessPortalWidgetsHost** scheme (not the widget extension) at
   the top of the Xcode window.
2. Press **⌘R** to build and run. A small window titled "Success Portal
...
3. Click the date/time in the menu bar to open Notification Center, scroll to
   the bottom, click **Edit Widgets**, find **Success Portal Widgets** in the
   list, and drag in **Today's Tasks**, **Today's Workout**, and/or **Today's
   Meals**.

## 5. Keep the data fresh

The Electron app writes `~/Library/Application Support/Success Portal/widget-data.json`
...
minutes, so:
- Keep Success Portal open (or open it once in a while) to update the file.
- If a widget shows nothing, open Success Portal at least once so it writes
  the initial file.
...
- **File not found errors at runtime**: confirm the path really exists —
  `cat ~/Library/Application\ Support/Success\ Portal/widget-data.json` in
  Terminal after opening the Electron app once.
- Tapping a widget currently tries to open a `successportal://` URL, which
...
The Spotify widget lives on the **Widgets** page inside Success Portal itself
(not one of the Notification Center widgets above) and lets you see and
control what's playing. It needs a free Spotify Developer app to talk to
Spotify's API on your behalf:

1. Go to [developer.spotify.com/dashboard](https://developer.spotify.com/dashboard)
   and log in with your Spotify account.
2. Click **Create app**. Name it anything (e.g. "Success Portal"). App
...
6. In Success Portal, go to **Widgets**, paste the Client ID into the
...
Spotify Connect — it doesn't play audio itself. The sidebar mini-player's
expandable playlist view needs one more permission (reading playlist
contents) than the original release did — if you connected before that was
added, click **Disconnect** then **Connect Spotify** again to re-authorize
with the full permission set.

**Where the token is stored**: encrypted on disk (via Electron's
`safeStorage`, backed by macOS Keychain) at
`~/Library/Application Support/second-brain-os/spotify-tokens.enc` — this
path uses the app's internal package name, which is intentionally unchanged
from the Success Portal rename so existing connections keep working. Click
